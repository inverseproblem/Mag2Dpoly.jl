...
###########################################################################
#  Input:                                                                 #
#  Mind --> Magnetizzazione indotta   --> A/m                             #
#  Mrem --> Magnetizzazione rimanente --> A/m                             #
#  Iind --> Inclinazione magn. ind.   --> °                               #
#  Irem --> Inclinazione magn. rem.   --> °                               # 
#  Dind --> Declinazione magn. ind.   --> °                               #
#  Drem --> Declinazione magn. rem.   --> °                               #
#  Obs  --> Coordinate (x,z) relative ad ogni pto oss.
#  northxax --> orientazione profilo  --> °
###########################################################################
#  Output:                                                                #
#  tmag --> Campo magnetico scalare totale --> nT                         #
###########################################################################
...
function test1()
    ...
    nfwd = length(forwardtype)
    tmag = zeros(size(xzobs,1),nfwd)
    nbody = length(pbody.bo)
    for (f,fwdt) in enumerate(forwardtype)
        tmag[:,f] = tmagpolybodies2Dgen(xzobs,[Jind],[Jrem],northxax,pbody,fwdt)
 
        # for i=1:nbodya
        #     tmag[:,f] .+= tmagpoly2Dgen(xzobs,Jind,Jrem,northxax,pbody.bo[i],fwdt)
        # end
    end

    ##----------------------------------------------------------

    tmag_ref = [-116.16336912423309;
                -107.60475622349567;
                -98.17723975959588;
                -87.82662842366796;
                -76.49775256995866;	
    ...
    ##----------------------------------------------
    same = [all(tmag[:,i].≈tmag_ref) for i=1:size(tmag,2)]
    ...
    if all(same)
        ...
        for i=1:length(same)
            if same[i]==false
                println("Failed for forward type: $(forwardtype[i])")
            end
        end
        return false
    end
...
function test2()
    ...
    vertices  = [39.9767752377201 13.278991954030298;         
    ...
    nbody = length(pbody.bo)
    for (f,fwdt) in enumerate(forwardtype)
        tmag[:,f] = tmagpolybodies2Dgen(xzobs,[Jind,Jind,Jind],[Jrem,Jrem,Jrem],northxax,pbody,fwdt)
        # for i=1:nbody
        #     tmag[:,f] .+= tmagpoly2Dgen(xzobs,Jind,Jrem,northxax,pbody.bo[i],fwdt)
        # end
    end
